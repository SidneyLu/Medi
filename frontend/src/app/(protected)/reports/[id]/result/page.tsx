...
export default function ReportResultPage() {
  ...
  return <div className="page">
    ...
    <div className="grid two">
      ...
      <section className="panel">
        ...
        <div className="grid three panel-pad">
          <div className="profile-kv"><span>偏高</span><strong>{data.items.filter((item) => item.status === "high").length}</strong></div>
          <div className="profile-kv"><span>偏低</span><strong>{data.items.filter((item) => item.status === "low").length}</strong></div>
          <div className="profile-kv"><span>范围内</span><strong>{data.items.filter((item) => item.status === "normal").length}</strong></div>
        </div>
      </section>
    </div>
    ...
    {data.items.some((item) => item.explanation) && <section className="panel" style={{ marginTop: 18 }}>
      <header className="panel-head">
        <div>
          <h2>说明与建议</h2>
          <p>来自已确认指标的科普信息。</p>
      ...
      <div className="panel-pad">
        {data.items.filter((item) => item.explanation).map((item) => <div className="explanation" key={item.item_id}>
          <h3>{item.name}<span className={`status ${item.status}`}>{STATUS_LABEL[item.status]}</span></h3>
          <p>{item.explanation}</p>
          {!!item.suggestions?.length && <div className="tag-row" style={{ marginTop: 10 }}>{item.suggestions.map((suggestion) => <span className="tag neutral" key={suggestion}>{suggestion}</span>)}</div>}
        </div>)}
      </div>
    </section>}
  </div>;
