        ...
        <form className="panel panel-pad" onSubmit={form.handleSubmit((values) => save.mutate(values))}>
          <div className="grid" style={{ gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14 }}>
            <div className="field">
              ...
              {form.formState.errors.nickname && <span className="field-error">{form.formState.errors.nickname.message}</span>}
            </div>
            <div className="field">
              <label>出生日期</label>
              <input type="date" min="1900-01-01" max="2026-12-31" {...form.register("birth_date")} />
              {form.formState.errors.birth_date && <span className="field-error">{form.formState.errors.birth_date.message}</span>}
            </div>
            <div className="field">
              <label>出生性别</label>
              <select {...form.register("sex_at_birth")}>
            ...
            <div className="field">
              <label>妊娠状态</label>
              <select {...form.register("pregnancy_status")}>
            ...
            <div className="field">
              ...
              <select {...form.register("smoking_status")}>
                ...
                <option value="former">既往吸烟</option>
                <option value="current">当前吸烟</option>
              </select>
            </div>
            <div className="field">
              <label>饮酒状态</label>
              <select {...form.register("alcohol_use")}>
                <option value="unknown">未说明</option>
            ...
            <div className="field">
              <label>运动水平</label>
              <select {...form.register("exercise_level")}>
                <option value="unknown">未说明</option>
                <option value="low">较少运动</option>
                <option value="moderate">规律运动</option>
                <option value="high">高频运动</option>
              </select>
            </div>
            <div className="field">
              <label>睡眠质量</label>
              <select {...form.register("sleep_quality")}>
                <option value="unknown">未说明</option>
            ...
            <div className="field" style={{ gridColumn: "1 / -1" }}>
              <label>饮食模式</label>
              <select {...form.register("diet_pattern")}>
                <option value="unknown">未说明</option>
                <option value="balanced">相对均衡</option>
                <option value="high_salt">偏高盐</option>
                <option value="high_sugar">偏高糖</option>
                <option value="high_fat">偏高脂</option>
          ...
          <div className="field">
            <label>已知慢性病或长期健康情况</label>
            <textarea placeholder="例如：高血压，过敏性鼻炎（用逗号分隔）" {...form.register("chronic_conditions")} />
          </div>
          <div className="field">
            <label>过敏史</label>
            <textarea placeholder="例如：青霉素（用逗号分隔，不确定请留空）" {...form.register("allergies")} />
          </div>
          <div className="field">
            <label>当前常用药</label>
            <textarea placeholder="例如：氯雷他定（用逗号分隔，不确定请留空）" {...form.register("current_medications")} />
          </div>
          <div className="field">
            <label>家族史</label>
            <textarea placeholder="例如：糖尿病，高血压，冠心病（用逗号分隔）" {...form.register("family_history")} />
          </div>
          <div className="field">
            <label>近期症状</label>
            <textarea placeholder="例如：头晕，乏力，咳嗽（用逗号分隔）" {...form.register("recent_symptoms")} />
          </div>

          {save.isSuccess && (
            <div className="notice">
              <Check size={18} />
              <span>已保存。后端已重新生成画像标签和关键词。</span>
            </div>
          )}
          <button className="primary-button" type="submit" disabled={save.isPending}>
            {save.isPending ? "正在保存…" : "保存健康画像"}
          </button>
        </form>
        ...
        <aside className="grid" style={{ alignContent: "start" }}>
          <section className="panel panel-pad">
            ...
            <p className="subcopy" style={{ margin: 0 }}>
              后端会先把结构化画像转换为标签，再通过关键词算法提取高优先级健康关键词；AI 只接收这些摘要上下文，不需要直接读取完整表单。
            </p>
          </section>
          <section className="panel panel-pad">
            ...
            <div className="tag-row">
              {profile.data?.tags.length
                ...
                : <span className="tag neutral">保存后生成</span>}
            </div>
          </section>
          <section className="panel panel-pad">
            <h2 style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 17, marginTop: 0 }}>
              <Sparkles size={17} />
              画像关键词
            </h2>
            <div className="tag-row">
              {profile.data?.keywords.length
                ? profile.data.keywords.map((item) => (
                  <span
                    className="tag"
                    title={`${item.category} · ${item.source} · ${item.score}`}
                    key={`${item.category}-${item.keyword}`}
                  >
                    {item.keyword}
                  </span>
                ))
                : <span className="tag neutral">保存后提取</span>}
            </div>
          </section>
        </aside>
      </div>
